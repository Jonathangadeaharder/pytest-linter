...
impl Config {
    ...
    /// Create a Config with all 28 rules enabled by default and no overrides
    pub fn default() -> Self {
    ...
    /// Check if a specific rule is enabled. Unknown rules are treated as disabled.
    ...
    pub fn from_pyproject(dir: &Path) -> Result<Option<Self>> {
        ...
        loop {
            ...
            if candidate.exists() {
                ...
                let full: toml::Value = toml::from_str(&contents).with_context(|| {
                    format!("parse TOML in {}", candidate.display())
                })?;
                ...
                if tool_table.as_table().unwrap().is_empty() {
                    return Ok(None);
                }

                // Serialize the value back to string so we can deserialize into ToolConfig
                let tool_str = toml::to_string(&tool_table).unwrap_or_default();
                let tool_config: ToolConfig = toml::from_str(&tool_str).with_context(|| {
                    format!("deserialize tool.pytest-linter from {}", candidate.display())
                })?;
                ...
                if tool_config.output.is_some() {
                    cfg.output = tool_config.output;
                }
...
mod tests {
    ...
    fn test_default_has_28_rules_enabled() {
        ...
        for rid in Config::default_rule_ids() {
            assert!(cfg.is_rule_enabled(rid), "rule {} should be enabled by default", rid);
        }
    }
    ...
    fn test_rule_severity_override_and_default() {
        ...
        cfg.rules.insert(
            ...
            RuleConfig { enabled: Some(true), severity: Some(Severity::Info) },
        );
        assert_eq!(cfg.rule_severity("PYTEST-FLK-001", Severity::Warning), Severity::Info);
        // Unknown rule should return the provided default
        assert_eq!(cfg.rule_severity("UNKNOWN-001", Severity::Error), Severity::Error);
    }
    ...
    fn test_from_pyproject_parses_valid_toml() {
        ...
        let dir = std::env::temp_dir().join("pytest_linter_pyproject_test");
...
severity = "info"
"#;
        std::fs::write(dir.join("pyproject.toml"), toml_content).unwrap();

        let cfg = Config::from_pyproject(&dir).unwrap().unwrap();
        // format/output overrides
        assert_eq!(cfg.format, Some("json".to_string()));
        assert_eq!(cfg.output, Some(PathBuf::from("report.json")));
