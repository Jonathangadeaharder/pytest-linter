...
impl Config {
    /// Rule IDs enabled by default. Derived from the canonical rule registry
    /// (`all_rules()`) so this list can never drift from the implemented rules.
    fn default_rule_ids() -> Vec<&'static str> {
        crate::rules::all_rules().iter().map(|r| r.id()).collect()
    }
...
mod tests {
    ...
    #[test]
    fn test_default_has_all_rules_enabled() {
        let cfg = Config::default();
        assert_eq!(cfg.rules.len(), crate::rules::all_rules().len());
